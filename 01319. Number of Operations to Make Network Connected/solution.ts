 ...
 * @param connections - Array of connections [a, b]
 *
 * @returns Minimum cables needed or -1 if impossible
 */
const makeConnected = (n: number, connections: number[][]): number => {
  ...
  let remainingConnectionsNeeded: number = n - 1
  // Count of redundant connections (connections that create cycles)
  let redundantConnections: number = 0
  ...
  connections.forEach(([nodeA, nodeB]) => {
    ...
    const rootA: number = find(nodeA),
      rootB: number = find(nodeB)
