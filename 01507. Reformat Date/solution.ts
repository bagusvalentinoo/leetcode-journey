 ...
 *
 * @param date - Input date string (e.g., "20th Oct 2052")
 *
 * @returns Reformatted date in YYYY-MM-DD format
 */
const reformatDate = (date: string): string => {
  ...
  const dayString: string = dateParts[0].slice(0, -2).padStart(2, '0')
  ...
  const monthString: string = monthMap[dateParts[1]]

  // Extract year component
  const yearString: string = dateParts[2]
