/**
 * Problem: 1415. The k-th Lexicographical String of All Happy Strings of Length
 * n
 *
 * Difficulty: Medium
 *
...
public:
  string getHappyString(int n, int k) {
    // Calculate number of happy strings for each starting letter at current
    // position For length n, each starting letter has 2^(n-1) possible
    // combinations
    int groupSize = 1 << (n - 1);
    ...
    vector<string> nextOptions = {"bc", "ac", "ab"};
    ...
    for (int position = 1; position < n; position++) {
      ...
      string options = nextOptions[result.back() - 'a'];
