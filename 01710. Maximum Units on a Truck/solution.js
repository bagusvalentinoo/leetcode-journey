 ...
 *
 * @param {number[][]} boxTypes - Array of [boxCount, unitsPerBox] pairs
 * @param {number} truckSize - Maximum boxes that fit on truck
 *
 * @returns {number} Maximum total units loaded on truck
 */
const maximumUnits = (boxTypes, truckSize) => {
  // Track total units loaded and maximum units per box
  let totalUnits = 0, maxUnits = -1

  // Find maximum units per box to size frequency array
  for (let boxIndex = 0; boxIndex < boxTypes.length; boxIndex++) {
    if (boxTypes[boxIndex][1] > maxUnits) maxUnits = boxTypes[boxIndex][1]
  }

  // Build frequency of box counts by units per box
  const unitFrequencies = new Array(maxUnits + 1).fill(0)

  // Accumulate box counts for each units per box value
  for (let boxIndex = 0; boxIndex < boxTypes.length; boxIndex++) unitFrequencies[boxTypes[boxIndex][1]] += boxTypes[boxIndex][0]

  // Take boxes greedily starting from highest unit value
  for (let units = maxUnits; units >= 1; units--) {
    // Stop when truck is full
    if (truckSize === 0) break

    // Take as many boxes of this unit value as fit
    const take = Math.min(unitFrequencies[units], truckSize)
