 ...
 *
 * @param words - Array of words to sum
 * @param result - Result word
 *
 * @returns True if puzzle is solvable
 */
const isSolvable = (words: string[], result: string): boolean => {
  // Combine all words and result into a single array
  const allWords: string[] = [...words, result]

  // Get unique letters from all words and result
  const letters: string[] = [...new Set(allWords.join(''))]
  ...
  const nonZero: Set<string> = new Set()
  ...
  const coefficient: Map<string, number> = new Map()
  ...
  letters.sort(
    (a: string, b: string) =>
      Math.abs(coefficient.get(b)!) - Math.abs(coefficient.get(a)!)
  )

  // Number of unique letters
  const letterCount: number = letters.length

  // Array of coefficients in same order as sorted letters
  const coeffArray: number[] = letters.map(
    (char: string) => coefficient.get(char)!
  )

  // Precompute suffix sums for pruning bounds
  const suffixMax: number[] = new Array(letterCount + 1).fill(0)
  const suffixMin: number[] = new Array(letterCount + 1).fill(0)

  // Calculate suffix bounds from right to left
  for (let i: number = letterCount - 1; i >= 0; i--) {
    // Current coefficient value
    const currentCoeff: number = coeffArray[i]
  ...
  const backtrack = (index: number, currentSum: number): boolean => {
    ...
    const currentChar: string = letters[index]

    // Coefficient for current letter
    const currentCoeff: number = coeffArray[index]

    // Starting digit (0 unless letter is leading and cannot be zero)
    const startDigit: number = nonZero.has(currentChar) ? 1 : 0

    // Try all possible digits for current letter
    for (let digit: number = startDigit; digit <= 9; digit++) {
